import typing_var
from graphics import game_out, char_stats, typing_animation
...
def narrative_read(identifier:str, tag = "blue"):
    with open("src/narrative.txt") as narrative:
...
def opening_combat():
    global player
    from combat import combat_order
...
def new_ability(text): #gamestate 8
    from abilities import new_ability_dict
    global available_abilities, gamestate
    available_abilities = []
    for available_style in new_ability_dict[player.player_class][f"{player.player_class}_Styles"]:
        if available_style not in [style.name for style in player.styles]:
            available_abilities.append(available_style)
    for available_spell in new_ability_dict[player.player_class][f"{player.player_class}_Spells"]:
...
These styles are available to you: {[new_ability for new_ability in available_abilities if new_ability in new_ability_dict[player.player_class][f"{player.player_class}_Styles"]]}
These spells are available to you: {[new_ability for new_ability in available_abilities if new_ability in new_ability_dict[player.player_class][f"{player.player_class}_Spells"]]}"

Make a selection by typing in the name of the new ability you want to learn.''', "blue")
    gamestate = 9
      
def choose_new_ability(text): #gamestate 9
    from abilities import Ability, Style, Spell
    ...
    player.level_up()
    for new_ability in Ability.ability_list:
        if text.title() == new_ability.name and isinstance(new_ability, Style):
            player.styles.append(new_ability)
            game_out(f"You have learned a new style: {new_ability.name}", "purple_bold")
        elif text.title() == new_ability.name and isinstance(new_ability, Spell):
...
def post_grove_battle(text): #12
    from combat import regenerate_resources
